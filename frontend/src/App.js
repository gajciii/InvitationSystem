...
const responseTokenKey = (invitationId) =>
  `${RESPONSE_TOKEN_PREFIX}${invitationId}`;
...
const buildResponsesFilename = (title) => {
  const slug = (title || "invitation")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
  return `${slug || "invitation"}-responses.pdf`;
};
...
function App() {
  ...
  const [responseToken, setResponseToken] = useState(() =>
    readStoredResponseToken(invitationIdFromUrl)
  );
  ...
  const [responsesSort, setResponsesSort] = useState("updated_desc");
  ...
  const managedInvitationIdRef = React.useRef("");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let url =
        mode === "register" ? `${AUTH_URL}/register` : `${AUTH_URL}/login`;
      let body =
        mode === "register"
  ...
  const refreshMyResponses = React.useCallback(async () => {
    ...
    try {
      const res = await fetch(`${INV_URL}/my/responses`, {
  ...
  React.useEffect(() => {
    const currentManagedId = managedInvitation?._id || "";
    if (managedInvitationIdRef.current !== currentManagedId) {
      setResponsesFilter("all");
      setResponsesSort("updated_desc");
      setResponsesMessage("");
      setResponsesError("");
      managedInvitationIdRef.current = currentManagedId;
    }
  ...
  const saveMyResponse = async (invitationId) => {
    ...
    setMyResponses((prev) =>
      prev.map((item) =>
        item.invitationId === invitationId
          ? { ...item, saving: true, responseError: "", message: "" }
          : item
      )
    );
    try {
        ...
        prev.map((item) =>
          item.invitationId === invitationId
            ? {
                ...item,
                saving: false,
                responseError: err.message || "Failed to update response",
              }
            : item
        )
      );
  ...
  const deleteCurrentInvitation = async () => {
    if (!managedInvitation || !token) return;
    if (
      !window.confirm(
        "Delete this invitation? This will remove all associated responses."
      )
    )
      return;
    setResponsesMessage("");
    ...
    try {
      ...
      const removedId = managedInvitation._id;
      setMyInvitations((prev) => {
        const updated = prev.filter((inv) => inv._id !== removedId);
        setManagedInvitation((current) => {
          if (current && current._id !== removedId) return current;
      ...
      setResponsesError(err.message || "Failed to delete invitation");
    } finally {
  ...
  const exportResponsesPdf = async () => {
    ...
    try {
      const res = await fetch(`${INV_URL}/${managedInvitation._id}/export`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (!res.ok) {
        let data = {};
      ...
      link.href = url;
      link.download = buildResponsesFilename(
        managedInvitation.title || managedInvitation._id
      );
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.URL.revokeObjectURL(url);
      setResponsesMessage("PDF downloaded");
    } catch (err) {
      setResponsesError(err.message || "Failed to export responses");
    } finally {
  ...
  const submitRsvp = async () => {
    ...
    if (!publicCanEdit) {
      setError("RSVP window is closed");
      return;
    }
    setError("");
    setSavingRsvp(true);
    ...
    try {
      ...
      setRsvpSaved(
        data.mode === "updated" ? "Response updated" : "Response submitted"
      );
      if (token) {
        refreshMyResponses();
      }
  ...
  const ownerFilteredResponses = React.useMemo(() => {
    if (!managedInvitation?.responses) return [];
    const nameOf = (resp) => (resp?.name && resp.name.trim()) || "Anonimno";
    const dateValue = (resp) =>
      new Date(resp?.updatedAt || resp?.createdAt || 0).getTime();
    const baseList =
      responsesFilter === "all"
        ? [...managedInvitation.responses]
        : managedInvitation.responses.filter(
            (resp) => resp.status === responsesFilter
          );
    return baseList.sort((a, b) => {
      switch (responsesSort) {
        case "name_asc":
          return nameOf(a).localeCompare(nameOf(b), undefined, {
            sensitivity: "base",
          });
        case "name_desc":
          return nameOf(b).localeCompare(nameOf(a), undefined, {
            sensitivity: "base",
          });
        case "status":
  ...
  }, [managedInvitation, responsesFilter, responsesSort]);
      ...
      >
        ...
        {createdInvitationId && (
          <div style={{ fontSize: 12 }}>
            Share:{" "}
            <a href={getShareUrl(createdInvitationId)}>
              {getShareUrl(createdInvitationId)}
            </a>
            <button
              onClick={() =>
                navigator.clipboard.writeText(getShareUrl(createdInvitationId))
              }
  ...
  const renderEditPanel = () => (
    <div style={{ padding: 16 }}>
      {!managedInvitation && (
        <div style={{ color: "#888" }}>Select an invitation to edit</div>
      )}
      {managedInvitation && (
        <>
          ...
          >
            ...
            <input
              value={editRsvpLink}
              onChange={(e) => setEditRsvpLink(e.target.value)}
              placeholder="RSVP Link (optional)"
              style={{ padding: 10, borderRadius: 8, border: "1px solid #ddd" }}
            />
            <button
            ...
            {editSuccess && (
              <div style={{ color: "#22c55e" }}>{editSuccess}</div>
            )}
          </div>
        </>
      )}
  ...
  const renderResponsesPanel = () => (
    <div style={{ padding: 16 }}>
      {!managedInvitation && (
        <div style={{ color: "#888" }}>
          Select an invitation to view Odgovori
        </div>
      )}
      {managedInvitation && (
        <>
          ...
          >
            <div>
              ...
              <div style={{ fontSize: 12, color: "#666" }}>
                {formatDateTime(managedInvitation.dateTime)}
              </div>
            </div>
            <div style={{ fontSize: 12, color: "#666" }}>
              Showing {ownerFilteredResponses.length} of{" "}
              {managedInvitation.responses?.length || 0} responses
            </div>
          </div>
          <div style={{ marginTop: 10, fontSize: 12 }}>
            Share:{" "}
            <a href={getShareUrl(managedInvitation._id)}>
              {getShareUrl(managedInvitation._id)}
            </a>
            <button
              onClick={() =>
          ...
          {managedInvitation.responseCutoff && (
            <div style={{ marginTop: 8, fontSize: 13, color: "#2563eb" }}>
              RSVP cutoff:{" "}
              {new Date(managedInvitation.responseCutoff).toLocaleString()}
            </div>
          )}
          <div
            style={{
          ...
          >
            ...
            >
              ...
              <div style={{ fontSize: 20, fontWeight: 700 }}>
                {ownerResponseCounts.not_attending}
              </div>
            </div>
          </div>
          <div
          ...
          >
            ...
            <select
              value={responsesSort}
              onChange={(e) => setResponsesSort(e.target.value)}
              style={{
                padding: "6px 10px",
                borderRadius: 8,
                border: "1px solid #ddd",
            ...
            >
              {exportingResponses ? "Preparing PDF..." : "Export PDF"}
            </button>
            <button
              ...
              style={{
                ...
                borderRadius: 8,
                border: 0,
                background: deletingInvitation ? "#fecaca" : "#dc2626",
                color: "#fff",
              }}
            >
          ...
          {(responsesMessage || responsesError) && (
            <div
              style={{
                marginTop: 8,
                color: responsesError ? "#b91c1c" : "#15803d",
                fontSize: 13,
              }}
            >
              {responsesError || responsesMessage}
            </div>
          )}
          {ownerFilteredResponses.length === 0 && (
          ...
          {ownerFilteredResponses.length > 0 && (
            <div style={{ marginTop: 20 }}>
              ...
              {ownerFilteredResponses.map((r, idx) => (
                <div
                  key={r._id || idx}
                  style={{
                    display: "grid",
                  ...
                  <div>
                    <span
                      style={{
                        ...
                        borderRadius: 999,
                        fontSize: 12,
                        background: statusBadgeColor(r.status) + "22",
    ...
    return (
      <div>
        ...
        >
          ...
          {publicInvitation.dateTime && (
            <div style={{ fontSize: 13, color: "#666", marginTop: 6 }}>
              {new Date(publicInvitation.dateTime).toLocaleString()}
            </div>
          )}
          {publicInvitation.location && (
            <div style={{ fontSize: 13, color: "#666", marginTop: 2 }}>
              {publicInvitation.location}
            </div>
          )}
          {publicInvitation.message && (
            <p style={{ marginTop: 14 }}>{publicInvitation.message}</p>
          )}
          {publicInvitation.responseCutoff && (
            <div
              style={{
                marginTop: 12,
                fontSize: 12,
                color: publicCanEdit ? "#2563eb" : "#b91c1c",
              }}
            >
              RSVP cutoff:{" "}
              {new Date(publicInvitation.responseCutoff).toLocaleString()}
            </div>
          )}
          <div style={{ marginTop: 16, textAlign: "left" }}>
            ...
            <button
              onClick={submitRsvp}
              disabled={!publicCanEdit || savingRsvp}
              style={{
                marginTop: 10,
                padding: "10px 16px",
                background: publicCanEdit ? "#2563eb" : "#94a3b8",
                color: "#fff",
                border: 0,
                borderRadius: 8,
              }}
            ...
            {rsvpSaved && (
              <div style={{ color: "#16a34a", marginTop: 8 }}>{rsvpSaved}</div>
            )}
            {myResponse && (
              <div style={{ marginTop: 10, fontSize: 12, color: "#666" }}>
                Last updated:{" "}
                {myResponse.updatedAt
                  ? new Date(myResponse.updatedAt).toLocaleString()
                  : "just now"}
              </div>
            )}
          </div>
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {myInvitations.map((inv) => (
                <div
                  ...
                  style={{
                    ...
                    padding: 10,
                    background:
                      managedInvitation?._id === inv._id ? "#eef2ff" : "#fff",
                  }}
                >
                  <button
                    onClick={() => setManagedInvitation(inv)}
                    style={{
                      textAlign: "left",
                      width: "100%",
                  ...
                  <div style={{ marginTop: 6, fontSize: 12 }}>
                    Link:{" "}
                    <a href={getShareUrl(inv._id)}>{getShareUrl(inv._id)}</a>
                    <button
                      onClick={() =>
                        navigator.clipboard.writeText(getShareUrl(inv._id))
                      }
                      style={{
                        marginLeft: 8,
                        padding: "2px 6px",
              ...
              {myInvitations.length === 0 && (
                <div style={{ color: "#888" }}>No invitations yet</div>
              )}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {myResponses.map((resp) => {
                const saveDisabled =
                  !resp.canEdit ||
                  resp.saving ||
                  (resp.draftStatus === resp.status &&
                    resp.draftNotes === resp.notes);
                return (
                  <div
                    key={resp.invitationId}
                    style={{
                  ...
                  >
                      ...
                      <div style={{ fontSize: 12, textAlign: "right" }}>
                        ...
                        <span
                          style={{
                            padding: "3px 8px",
                            borderRadius: 999,
                            background: statusBadgeColor(resp.status) + "22",
                            color: statusBadgeColor(resp.status),
                          }}
                        >
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                      ...
                      <button
                        onClick={() => {
                          window.location.href = getShareUrl(resp.invitationId);
                        }}
                        style={{
                          flex: 0.15,
                          minWidth: 80,
                    ...
                    <button
                      ...
                      style={{
                        ...
                        border: 0,
                        color: "#fff",
                        background: saveDisabled ? "#93c5fd" : "#2563eb",
                      }}
                    >
                    ...
                    {resp.responseCutoff && (
                      <div
                        style={{
                          marginTop: 6,
                          fontSize: 11,
                          color: resp.canEdit ? "#2563eb" : "#b91c1c",
                        }}
                      >
                        RSVP cutoff:{" "}
                        {new Date(resp.responseCutoff).toLocaleString()}
                      </div>
                    )}
                    {!resp.canEdit && (
                      <div
                        style={{ fontSize: 11, color: "#b91c1c", marginTop: 2 }}
                      >
                        Changes are closed for this event.
                      </div>
                    )}
                    {resp.message && (
                      <div
              ...
              {!myResponsesError && myResponses.length === 0 && (
                <div style={{ color: "#888" }}>You have not responded yet</div>
              )}
            </div>
          </div>
